...
const exercise = whoIsBigger(23,7)
...
const string = "Esercizi settimana 3"
...
function searchByTitle(movies, string3){
  return movies.filter(movie => movie.Title.includes(string3));
}
...
/* ESERCIZIO 18
  ...
  stringa fornita all'interno del proprio titolo, mentre "unmatch" deve 
  includere tutti i rimanenti.
*/

function searchAndDivide(movies, string4){
  const match = movies.filter(movie => movie.Title.includes(string4));
  const unmatch = movies.filter(movie => !movie.Title.includes(string4));
...
/* ESERCIZIO 19
  ...
  ritorna l'array "movies" fornito privo dell'elemento nella posizione ricevuta 
  come parametro.
*/
